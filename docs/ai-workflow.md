...
Две заявленные деградации — если события пропали, причина обычно здесь:

- **`PR_REVIEW` не приходит вовсе.** Старый `gh` не знает поле
  `reviewDecision`; watcher один раз сообщает об этом строкой
  `WATCH_ERROR list_prs: gh не знает поле reviewDecision` и дальше
  отслеживает всё остальное. Лечится обновлением `gh`.
- **`PR_CLOSED` не приходит, пока открытых PR больше 200.** У `gh pr list`
  нет `--paginate`, поэтому список упирается в лимит; на заведомо неполном
  снимке отсутствие PR в ответе означает «его не видно», а не «он закрыт».
  Закрытия на таких итерациях пропускаются намеренно — ложное `PR_CLOSED`
  необратимо (watcher теряет состояние PR и потом врёт ещё и `PR_OPENED`).
  Сообщается строкой `WATCH_ERROR list_prs: снимок усечён лимитом`.
